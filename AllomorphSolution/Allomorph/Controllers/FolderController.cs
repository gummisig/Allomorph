    ...
    public class FolderController : Controller
    {
        ...
        public ActionResult TextEdit(int? id)
        {
            ...
            foreach(var item in lines)
            {
                ...
                var eng = from j in line
                          where j.LanguageID == 1
                          select j;

                var isl = from k in line
                          where k.LanguageID == 2
                          select k;
                
                if(eng.FirstOrDefault() == null)
                {
                    English.Add("");
                }
                else
                {
                    English.Add(eng.First().LineText);
                }

                if (isl.FirstOrDefault() == null)
                {
                    Icelandic.Add("");
                }
                else
                {
                    Icelandic.Add(isl.First().LineText);
                }
            }
